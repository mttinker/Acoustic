...
pathout = paste0(getwd(),"/",species,"_summaries/")

# Diagnostic Plots ------------------------------------------------------------------

if (data_opt==1){pfp=8}else{pfp=13}
for (i in 1:pfp){
  parnm = params[i]
  traplot(out,parnm)
}
...
if (data_opt==1){stratparnum=8}else{stratparnum=13}
...
if (data_opt==1){siteparnum=9}else{siteparnum=14}          
...
for (i in 1:Nfocalsites){
  ...
  for (j in 1:Nsmp){
    predmat[,j] = Ceff*Tmpeff[,j]*exp(Thta*Moonsmp[j])
  }
